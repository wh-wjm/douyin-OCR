...
抖音登记OCR 是一个用于批量识别抖音数据截图并导出登记表格的桌面工具。它可以从数字命名的图片目录中读取截图，自动识别直播数据和视频数据，并分别生成 `直播.csv` 与 `视频.csv`。

这个项目同时提供可复用的 Rust 库、命令行示例和 Slint 桌面界面。桌面程序面向普通使用者，库和示例面向需要二次开发、自动化处理或让大语言模型接手维护的人。
...
- 桌面界面支持选择目录、选择模型、暂停、继续、中止、查看关于页和点击开源项目链接。
...
从发布页面下载对应系统的压缩包。macOS 解压后运行 `抖音登记OCR.app`，Windows 解压后运行 `抖音登记OCR.exe`。

首次运行时，如果本机没有对应模型，程序会自动下载模型。下载完成后会继续处理图片。模型会存放在用户目录下的应用数据目录中，不会写入程序安装目录。

程序目前发布的是未签名版本。Windows 可能会出现安全提醒，macOS 可能需要在系统的隐私与安全性中手动允许运行。
...
```bash
cargo run --bin export_gui
cargo run --bin test_model
cargo run --example export_csv -- 图片目录
cargo check --bin export_gui
cargo test
```
...
推送形如 `v0.1.0` 的标签会触发发布流程。当前自动构建以下未签名压缩包：

- macOS Apple 芯片版本，压缩包内是 `抖音登记OCR.app`
- Windows x64 版本，压缩包内是 `抖音登记OCR.exe`
...
| `src/bin/export_gui.rs` | Slint 桌面程序 |
...
修改桌面界面时，主要改 `src/bin/export_gui.rs`。关于页中的链接通过 `webbrowser` 打开，模型下载进度通过 `ExportEvent::ModelDownload` 从库层传到界面。

如果要发布新版本，先确认 `cargo test` 通过，然后提交代码，创建并推送新的版本标签。
